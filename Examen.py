...
while True:
  ...
  if len(sys.argv) != nvar+1:
    sys.exit(f'El número de articulaciones no es el correcto ({nvar})')
  ...
  T341=matriz_T(d[3],th[3],a[3],a[3])
  T041=np.dot(T03,T341)

  T342=matriz_T(d[4],th[4],a[4],a[4])
  T042=np.dot(T03,T342)

  T34EE=matriz_T(d[5],th[5],a[5],a[5])
  T04EE=np.dot(T03,T34EE)
  ...
  o410 =np.dot(T041, o441).tolist()
  o420 =np.dot(T042, o442).tolist()
  o4EE =np.dot(T04EE, o4EE).tolist()

  print(o4EE)

  # Mostrar resultado de la cinemática directa
  listaOrigenes = [o00, o10,o20, o30, [[o410], [o420]], o4EE]
  muestra_origenes(listaOrigenes)
  muestra_robot   (listaOrigenes, ef=o4EE)

  for i in range(nvar):
      val = input(f"Introduce nuevo valor de la variable articular {i+1}: ")
      if (val != ""):
        p[i] = float(val)
  print("")
