...
def calcularError(distanciaIdealBaliza, distanciaRealBaliza):
  return
  ...
  for i  in range(0, 10):
    for j in range(0, 10):
      for baliza in balizas:
        distanciaIdealBaliza = [ideal.x - baliza.x, ideal.y - baliza.y]
        distanciaRealBaliza = [real.x - baliza.x, real.y - baliza.y]
        error.insert(distanciaIdealBaliza - distanciaRealBaliza)
      imagen[i][j] = [error, [x_real, y_real]]
      x_real+1
    y_real+1
